...
# Merge submissions
subs_to_merge = {
    '4000 trees power bill vars 2nd set depth 8.csv': .06,
    'upweighted 4000 trees.csv': .1,
    'upweighted 6500 trees no compset.csv': .07,
    'downweighted 4000 trees.csv': .06,
    'downweighted 6500 trees w compset.csv': .01,
    '4000 trees power bill vars.csv': .03,
    '4000 trees power no log.csv': .02,
    '2500 trees power bill vars.csv': .04,
    'weight 2500 trees.csv': .11,
    'downweighted 2500 trees.csv': .035,
    'threeway vars with bill vars.csv': .33,
    'threeway vars with subsamp.csv': .07,
    'threeway vars with forest.csv': .005,
    '1500 trees xgb.csv': .01,
    '2500 trees xgb.csv': .01,
    '2500 trees xgb w spec vars.csv': .005,
    '2500 trees xgb w extra vars.csv': .005,
    'stacking with xgboost second stage all vars.csv': .005,
    'stacking with all vars in forest.csv': .005,
    'xgboost from first data build.csv': .005,
    'stacking with higher eta.csv': .005,
    'stacking with ridge vars.csv': .005,
    'cv stack with new vars frst second stage internal 243.csv': .005
}
...
merge_subms(subs_to_merge, SUBM_PATH, 'blend x.csv', 'cost')

subm_correl('blend x.csv',
            'blend w.csv', 'id', 'cost')
